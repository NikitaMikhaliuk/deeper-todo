...
function handleAuthFormSubmit(evt) {
  ...
  const options = {
    ...
    body: `login=${encodeURIComponent(authForm.elements.login.value)}&password=${encodeURIComponent(authForm.elements.password.value)}`,
    credentials: 'include',
    redirect: 'follow',
  };
  fetch(authForm.action, options).then((res) => {
    if (res.status === 200) {
      return res.json();
    }
    ...
    if (res.status === 401) {
      ...
      document.getElementById('login').classList.add('input-incorrect-warning');
      document.getElementById('password').classList.add('input-incorrect-warning');
    }
  }).then((res) => {
    if (res) {
