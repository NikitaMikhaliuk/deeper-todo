...
export default class TodoCategoriesListItem extends Component {
  ...
  handleSubmitAddNestedCat = (catName) => {
    const linkPath = this.props.todoCategoryItem.linkPath + nameToUrl(catName);
  ...
  handleSubmitDelete = () => {
    ...
    this.props.actions.DeleteCategory(todoCategoryItem.parentCategoryId, id);
    this.setState({
      showDeleteModalDialog: false
    });
  }
  ...
  handleMoveItemInCategory = () => {
    this.props.actions.MoveTodoItem(this.props.chosenItemToEditId, this.props.id);
  }

  render() {
    const { todoCategoryItem, renderNestedList, selectedIndex, id, chosenItemToEditId } = this.props;
    const isSelected = selectedIndex === id;
    const { handleRenameCat, handleAddNestedCat, handleDeleteCat, handleSubmitDelete, handleCancelDelete } = this;
    const { showDeleteModalDialog, isEditing, showAddNestedCatForm } = this.state;
    const childToRender = isEditing ?
      ...
      : (<Link
          ...
          key={id + '_Link'}
          to={todoCategoryItem.linkPath  + (this.props.filter ? `&filter=${this.props.filter}` : '')}>
    ...
    const nestedItemsToRender = showAddNestedCatForm ?
      [ (
        ...
        innerDivStyle={{backgroundColor: isSelected ? grey400 : 'initial', paddingTop: '5px', paddingBottom: '5px'}}
        />
        )
      ].concat(nestedArr)
      : nestedArr;
    const leftIconToRender =  chosenItemToEditId && !todoCategoryItem.itemsIds.includes(chosenItemToEditId) ?
      (<IconButton
          style={{margin: '0'}}
      ...
      : OptionsMenu({ handleRenameCat, handleAddNestedCat, handleDeleteCat, disabled: todoCategoryItem.completed });
    return (
      <ListItem
        value={id}        
        key={id}
