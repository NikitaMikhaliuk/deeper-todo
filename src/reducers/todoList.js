...
function updateCategoriesProgress(categoryId, state) {
  ...
  while (chekingCatId !== 'root') {
    ...
    if(todoCat.itemsIds.every(itemId => newState.getIn(['itemsStorage', itemId, 'completed']))) {
      newState = newState.setIn(['categoriesStorage', chekingCatId, 'completed'], true);
      if(todoCat.categoriesIds.every(catId => !newState.getIn(['categoriesStorage', catId, 'visible']))
        && todoCat.itemsIds.length !== 0) {
          newState = newState.setIn(['categoriesStorage', chekingCatId, 'visible'], false);
      }
    } else {
      break;
    }
  ...
  const currentProgress = categoriesStorage
    .filter(catItem => catItem.toJS().completed)
    .size / categoriesStorage.size;
...
function rebuildLinkPaths(state, id, parentLinkPath) {
  ...
  newState = newState.setIn(['categoriesStorage', id, 'linkPath'], newLinkPath);
...
function addCategory(params, state) {
  ...
  const parentPathArr = parentCategoryId === 'root' ?
    ['root']
  ...
  let newState = state.setIn((parentPathArr.concat(['categoriesIds'])), parentNestedIds);
...
function deleteCategory(params, state) {
  ...
  const parentNestedCatsPathArr =  parentCategoryId === 'root' ?
    ['root', 'categoriesIds']
  ...
  const deleteCategory = (catId) => {         
    ...
    let nestedItemsIds = newState.getIn(['categoriesStorage', catId, 'itemsIds']).toJS();
    nestedItemsIds.forEach(itemId => {
      newState = newState.removeIn(['itemsStorage', itemId]);
    });

    let nestedCatsIds = newState.getIn(['categoriesStorage', catId, 'categoriesIds']).toJS();
...
function addTodoItem(params, state) {
  ...
  let parrentItemsIds = newState.getIn(['categoriesStorage', parentCategoryId, 'itemsIds']);
  parrentItemsIds = List([id]).concat(parrentItemsIds);
  newState = newState.setIn(['categoriesStorage', parentCategoryId, 'itemsIds'], parrentItemsIds);
  newState = newState.setIn(['categoriesStorage', parentCategoryId, 'completed'], false);
  newState = updateHistory(state, newState);
  return newState;
}
...
function EditTodoItem(params, state) {
  ...
  newState = newName ? newState.setIn(['itemsStorage', id, 'name'], newName) : newState;
  newState = newDescription ? newState.setIn(['itemsStorage', id, 'description'], newDescription) : newState;
  const parentCategoryId = newState.getIn(['itemsStorage', id, 'parentCategoryId']);
  newState = updateCategoriesProgress(parentCategoryId, newState);
  newState = updateHistory(state, newState);
  return newState;
}
...
function MoveTodoItem(params, state) {
  ...
  const parentCatId = state.getIn(['itemsStorage', itemId, 'parentCategoryId'])
  let parentItemsIds = state.getIn(['categoriesStorage', parentCatId, 'itemsIds']);
  parentItemsIds = parentItemsIds.filter(id => id !== itemId);
  let newState = state.setIn(['categoriesStorage', parentCatId, 'itemsIds'], parentItemsIds);
  let newParentItemsIds = state.getIn(['categoriesStorage', newParentCategory, 'itemsIds']);
  newParentItemsIds = List([itemId]).concat(newParentItemsIds);
  newState = newState.setIn(['categoriesStorage', newParentCategory, 'itemsIds'], newParentItemsIds);
  newState = newState.setIn(['itemsStorage', itemId, 'parentCategoryId'], newParentCategory);
