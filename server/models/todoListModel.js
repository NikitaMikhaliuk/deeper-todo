...
TodoListSchema.methods.addCategory = function (params) {
  ...
  if (todoCat.parentCategoryId !== 'root') {
    parentCategory = this.categoriesStorage.find(category => category.id === todoCat.parentCategoryId);
  } else {
...
TodoListSchema.methods.deleteEverythingInCategory = function (todoCatId) {
  const todoCatIndex = this.categoriesStorage.findIndex(category => category.id === todoCatId);
...
TodoListSchema.methods.deleteCategory = function (todoCatId) {
  const todoCat = this.categoriesStorage.find(category => category.id === todoCatId);
  const parentCategory = todoCat.parentCategoryId === 'root' ?
    this.root
    : this.categoriesStorage.find(category => category.id === todoCat.parentCategoryId);
  const todoCatIdIndex = parentCategory.categoriesIds.findIndex(id => id === todoCatId);
...
TodoListSchema.methods.renameCategory = function (params) {
  const todoCat = this.categoriesStorage.find(category => category.id === params.id);  
...
TodoListSchema.methods.updateTotalProgress = function () {  
  this.totalProgress = this.categoriesStorage.filter(cat => cat.completed).length/this.categoriesStorage.length;
}
  ...
  if(
    ...
    if (
      todoCat.categoriesIds.every(
        catId => !this.categoriesStorage.find(
          cat => cat.id === catId
        ).visible
...
TodoListSchema.methods.addTodoitem = function (params) {
  ...
  const parentCategory = this.categoriesStorage.find(category => category.id === todoItem.parentCategoryId);
...
TodoListSchema.methods.moveTodoItem = function (params) {
  ...
  const oldTodoCatIndex = this.categoriesStorage.findIndex(category => category.id === todoItem.parentCategoryId);
  const oldTodoCat = this.categoriesStorage[oldTodoCatIndex];
  
  const todoItemParentCatIndex = oldTodoCat.itemsIds.findIndex(item => item.id === params.itemId);
  oldTodoCat.itemsIds.splice(todoItemParentCatIndex, 1);
  const newTodoCat = this.categoriesStorage.find(category => category.id === params.newParentCategory);
